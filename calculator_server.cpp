...
// Глобальное хранилище переменных (состояние)
std::unordered_map<std::string, double> g_variables;
...
// Теперь он работает с глобальным состоянием переменных, переданным по ссылке.
class Parser {
public:
...
private:
    ...
    double parseStatement() {
        // Если встречается конструкция "идентификатор = выражение"
...
// Функция вычисления выражения с использованием глобального состояния
double evaluate_expression(const std::string& expression) {
    std::vector<Token> tokens = tokenize(expression);
    Parser parser(tokens, g_variables);
    return parser.parse();
}
...
void handle_request(tcp::socket& socket) {
    try {
        ...
        if (parsed_request.is_object()) {
            ...
            if (request_obj.contains("cmd") && request_obj["cmd"].is_string()) {
                ...
                try {
                    ...
                    // Если результат целое число, отправляем как int
        ...
        // Формирование ответа с пробелами между ключами и значениями
